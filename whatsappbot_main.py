...
def buscar_chats():
    ...
    for chat in chats:
        ...
        snapshots = firestore_db.collection(
            'contacto').where('nombre', '==', name).get()
        if not snapshots:
...
def enviar_foto(foto: str):
    print("foto :", foto)
    WebDriverWait(driver, timeout=3).until(lambda driver: driver.find_element_by_xpath(
        '//*[@id="main"]/footer/div[1]/div[1]/div[2]/div/div')).click()
    file_image = WebDriverWait(driver, timeout=10).until(lambda driver: driver.find_element_by_xpath(
        '//input[@accept="image/*,video/mp4,video/3gpp,video/quicktime"]'))
    file_image.send_keys(foto)
    WebDriverWait(driver, timeout=10).until(lambda driver: driver.find_element_by_xpath(
        '//*[@id="app"]/div[1]/div[1]/div[2]/div[2]/span/div[1]/span/div[1]/div/div[2]/span/div/div')).click()
...
def enviar_documento(documento: str):
    ...
    WebDriverWait(driver, timeout=3).until(lambda driver: driver.find_element_by_xpath(
        '//*[@id="main"]/footer/div[1]/div[1]/div[2]/div/div')).click()
    document = WebDriverWait(driver, timeout=3).until(
        lambda driver: driver.find_element_by_xpath('//input[@accept="*"]'))
...
def enviar_sticker(sticker: str):
    ...
    WebDriverWait(driver, timeout=3).until(lambda driver: driver.find_element_by_xpath(
        '//*[@id="main"]/footer/div[1]/div[1]/div[1]/button[2]')).click()
    sleep(1)
    WebDriverWait(driver, timeout=3).until(lambda driver: driver.find_element_by_xpath(
        '//*[@id="main"]/footer/div[1]/div[1]/div[1]/button[4]')).click()
    WebDriverWait(driver, timeout=3).until(lambda driver: driver.find_element_by_xpath(
        '//*[@id="main"]/footer/div[2]/div/div[3]/div[1]/div/div[1]/div[1]/div/div/div[2]')).click()
    sticker_path = '//*[@id="main"]/footer/div[2]/div/div[3]/div[1]/div/div[1]/div[2]/div/div[{}]'.format(
        sticker)
    #print("sticker_path :", sticker_path)
    WebDriverWait(driver, timeout=3).until(
        lambda driver: driver.find_element_by_xpath(sticker_path)).click()
    # Cierra
    WebDriverWait(driver, timeout=3).until(lambda driver: driver.find_element_by_xpath(
        '//*[@id="main"]/footer/div[1]/div[1]/div[1]/button[1]')).click()
...
def procesar_mensaje(message: str):
    ...
    for snapshot in snapshots:
        ...
        if message.__contains__(snap_dict.get("mensaje")):
            ...
            return
    enviar_respuesta(
        "Lo siento, no te comprendí :-( . Puedes explicarme mejor :-D")
